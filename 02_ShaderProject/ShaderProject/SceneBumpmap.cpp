...
struct TangentVtx
{
	...
	DirectX::XMFLOAT2 uv;
	// 新しくモデルに追加する頂点データ
	DirectX::XMFLOAT3 tangent;	// ワールド空間上でのテクスチャを貼り付ける横方向
};
...
void SceneBumpmap::Init()
{
	...
	auto calcTangent = [](Model::RemakeInfo& data)
	{
		...
		TangentVtx* destVtx =
			reinterpret_cast<TangentVtx*>(data.dest);
		...
		for (UINT i = 0; i < data.idxNum; i += 3) // 1面(3頂点、三角形)ずつ処理
		{
			// 三角形を構成する頂点情報の取得
			UINT idx0 = idx[i + 0];
			UINT idx1 = idx[i + 1];
...
void SceneBumpmap::Draw()
{
	...
	for (int i = 0; i < drawNum; ++i)
	{
		...
		for (int j = 0; j < meshNum; ++j)
		{
			...
			const TangentVtx* pVtx
				= reinterpret_cast<const TangentVtx*>(desc.pVtx);
